...
namespace Mushka.Tests.Service.Services
{
    public class CategoryServiceTest : ServiceTestBase
    {
        ...
        private const string UpdateAsyncMethodName = nameof(CategoryService.UpdateAsync) + ". ";
        ...
        private static readonly string CategoryCreatedMessage = $"Category with id {CategoryId} was successfully added.";
        private static readonly string CategoryUpdatedMessage = $"Category with id {CategoryId} was successfully updated.";
        ...
        public async Task AddAsyncTest()
        {
            ...
            categoryRepositoryMock
                ...
                .Setup(repo => repo.Add(category), category);
        ...
        public async Task UpdateAsyncTest()
        {
            ...
            categoryRepositoryMock
                ...
                .SetupAsync(repo => repo.IsExistAsync(It.IsAny<Expression<Func<Category, bool>>>(), default(CancellationToken)), false)
                .Setup(repo => repo.Update(category), category);

            storageMock
                .Setup(s => s.SaveAsync(default(CancellationToken)), Task.CompletedTask);

            var actual = await categoryService.UpdateAsync(category);

            var expected = CreateValidValidationResponse(category, CategoryUpdatedMessage);
            actual.Should().BeEquivalentTo(expected);
        }

        [Category(TestCategoryName)]
        [Fact(DisplayName = UpdateAsyncMethodName + "Category is not found")]
        public async Task UpdateAsyncCategoryNotFoundTest()
        {
            var category = CreateCategory();

            categoryRepositoryMock
                .SetupAsync(repo => repo.GetByIdAsync(CategoryId, default(CancellationToken)), null);
        ...
        [Category(TestCategoryName)]
        [Fact(DisplayName = UpdateAsyncMethodName + "Category name is duplicated")]
        ...
        public async Task DeleteAsyncTest()
        {
            ...
            categoryRepositoryMock
                .SetupAsync(repo => repo.GetByIdAsync(CategoryId, default(CancellationToken)), category)
                .Setup(repo => repo.Delete(category), category);
